...
fn main() {
    ...
    let kill_json = serde_json::to_value(&kill::KillResult {
        ok: true,
        killed: vec![1, 2],
        error: None,
    })
    ...
    let mut ok = true;
    for k in row_need {
        if row_json.get(k).is_none() {
            println!("✗ AppRow 缺字段: {k}");
            ok = false;
        }
    }
    // helper 字段
    if let Some(h) = row_json
        .get("helpers")
        ...
        .and_then(|a| a.first())
    {
        for k in ["name", "role", "cpu", "mem", "pid"] {
            if h.get(k).is_none() {
                println!("✗ Helper 缺字段: {k}");
                ok = false;
            }
        }
    }
    for k in stats_need {
        if stats_json.get(k).is_none() {
            println!("✗ SystemStats 缺字段: {k}");
            ok = false;
        }
    }
    for k in kill_need {
        if kill_json.get(k).is_none() {
            println!("✗ KillResult 缺字段: {k}");
            ok = false;
        }
    }
